...
=======
	bool y_dim = y1 - cube_l < y && y < y1 + cube_l;
	bool z_dim = z1 - cube_l < y && y < z1 + cube_l;
	return x_dim && y_dim && z_dim;
...
=======
	// these will be used to determine the sample area [we can use std::min()]
	double x_min = std::min(x1-cube_l, x2 - blue_r); 
	double x_max = std::max(x1+cube_l, x2+blue_r);

	double y_min = std::min(y1-cube_l,y2-blue_r);
	double y_max = std::max(y1+cube_l,y2+blue_r);
